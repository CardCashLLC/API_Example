...
return request(requestSessionOptions)
  ...
  .then( requestSellCardUpdateOptions => {

    console.log(`\n \nMAKING REQUEST WITH ${requestSellCardUpdateOptions.method} METHOD TO ${requestSellCardUpdateOptions.uri} WITH DATA ${JSON.stringify(requestSellCardUpdateOptions.body)} \n`);

    return request(requestSellCardUpdateOptions);

  })
  .then(result => {

    console.log(`Response from /v3/carts/${result.body.cartId}/cards/${result.body.cards[0].id}: \n`, result.body);
